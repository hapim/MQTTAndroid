 ...
 * {@link ModelConnectionPersistence} objects so created clients survive, the
 * destruction of the singleton {@link Connections} object.
 * 
 */
public class DatabaseConnectionPersistence extends SQLiteOpenHelper implements
		BaseColumns {

	/** The version of the database **/
	public static final int DATABASE_VERSION = 1;
	/**Tag for log*/
	public static final String TAG = "DatabaseConnectionPersistence";

	/** The name of the database file **/
	public static final String DATABASE_NAME = "mqttConnections.db";
	...
	/** Table column for contextid enabled **/
	public static final String COLUMN_ctxid = "ctxid";
	...
	"CREATE TABLE " + TABLE_CONNECTIONS + " (" + _ID + INT_TYPE + COMMA_SEP
			+ COLUMN_HOST + TEXT_TYPE + COMMA_SEP + COLUMN_client_ID
			+ TEXT_TYPE + COMMA_SEP + COLUMN_port + INT_TYPE + COMMA_SEP
			+ COLUMN_ctxid + TEXT_TYPE + COMMA_SEP + COLUMN_ssl + INT_TYPE
			+ COMMA_SEP + COLUMN_TIME_OUT + INT_TYPE + COMMA_SEP
			+ COLUMN_KEEP_ALIVE + INT_TYPE + COMMA_SEP + COLUMN_USER_NAME
			+ TEXT_TYPE + COMMA_SEP + COLUMN_PASSWORD + TEXT_TYPE + COMMA_SEP
			+ COLUMN_CLEAN_SESSION + INT_TYPE + COMMA_SEP + COLUMN_TOPIC
			+ TEXT_TYPE + COMMA_SEP + COLUMN_MESSAGE + TEXT_TYPE + COMMA_SEP
			+ COLUMN_QOS + INT_TYPE + COMMA_SEP + COLUMN_RETAINED
			+ " INTEGER, PRIMARY KEY (" + _ID + ")" + COMMA_SEP + UNIQUE + "("
			+ COLUMN_HOST + "," + COLUMN_client_ID + "," + COLUMN_port + "));";
	 ...
	 * 
	 * @param context
	 *            Context that the application is running in
	 */
	public DatabaseConnectionPersistence(Context context) {
	 ...
	 * @throws ConnectionPersistenceException
	 *             If storing the data fails
	 */
	public void persistConnection(ModelConnectionPersistence connection)
			throws ConnectionPersistenceException {
		/*
		 * SQLiteDatabase wrDb = getWritableDatabase();
		 * wrDb.execSQL(SQL_CREATE_ENTRIES);
		 */
		...
		values.put(COLUMN_ctxid, connection.getCtxId());
		...
		// uses "condition ? trueValue: falseValue" for in line converting of
		// values
		...
		values.put(COLUMN_CLEAN_SESSION, conOpts.isCleanSession() ? 1 : 0); // convert
																			...
																			// in
																			// values
		values.put(COLUMN_PASSWORD, password != null ? String.valueOf(password)
				: null); // convert char[] to String
		values.put(COLUMN_MESSAGE,
				lastWill != null ? new String(lastWill.getPayload()) : null); // convert
																				// byte[]
																				// to
																				// string
		...
		if (lastWill == null) {
			...
			values.put(COLUMN_RETAINED, lastWill.isRetained() ? 1 : 0); // convert
																		// from
																		// boolean
																		// to
																		// int
		...
		long newRowId = db.replace(TABLE_CONNECTIONS, null, values);
		Log.i(TAG,"replace new row id: "+ Long.toString(newRowId));
	 ...
	 * 
	 * @param context
	 *            Context for creating {@link ModelConnectionPersistence}
	 *            objects
	 * @return list of connections that have been restored
	 * @throws ConnectionPersistenceException
	 *             if restoring connections fails, this is thrown
	 */
	public List<ModelConnectionPersistence> restoreConnections(Context context)
			throws ConnectionPersistenceException {
		// columns to return
		String[] connectionColumns = { COLUMN_HOST, COLUMN_port,
				COLUMN_client_ID, COLUMN_ctxid, COLUMN_ssl, COLUMN_KEEP_ALIVE,
				COLUMN_CLEAN_SESSION, COLUMN_TIME_OUT, COLUMN_USER_NAME,
				COLUMN_PASSWORD, COLUMN_TOPIC, COLUMN_MESSAGE, COLUMN_RETAINED,
				COLUMN_QOS, _ID
		...
		Cursor c = db.query(TABLE_CONNECTIONS, connectionColumns, null, null,
				null, null, sort);
		ArrayList<ModelConnectionPersistence> list = new ArrayList<ModelConnectionPersistence>(
				c.getCount());
		ModelConnectionPersistence connection = null;
		for (int i = 0; i < c.getCount(); i++) {
			if (!c.moveToNext()) { // move to the next item throw persistence
									// exception, if it fails
				throw new ConnectionPersistenceException(
						"Failed restoring connection - count: " + c.getCount()
								+ "loop iteration: " + i);
			}
			...
			String clientID = c.getString(c
					.getColumnIndexOrThrow(COLUMN_client_ID));
			int port = c.getInt(c.getColumnIndexOrThrow(COLUMN_port));
			String ctx_id = c.getString(c.getColumnIndexOrThrow(COLUMN_ctxid));
			...
			String message = c.getString(c
					.getColumnIndexOrThrow(COLUMN_MESSAGE));

			// connect options integers
			int qos = c.getInt(c.getColumnIndexOrThrow(COLUMN_QOS));
			int keepAlive = c
					.getInt(c.getColumnIndexOrThrow(COLUMN_KEEP_ALIVE));
			int timeout = c.getInt(c.getColumnIndexOrThrow(COLUMN_TIME_OUT));

			// get all values that need converting and convert integers to
			// booleans in line using "condition ? trueValue : falseValue"
			boolean cleanSession = c.getInt(c
					.getColumnIndexOrThrow(COLUMN_CLEAN_SESSION)) == 1 ? true
					: false;
			boolean retained = c.getInt(c
					.getColumnIndexOrThrow(COLUMN_RETAINED)) == 1 ? true
					: false;
			boolean ssl = c.getInt(c.getColumnIndexOrThrow(COLUMN_ssl)) == 1 ? true
					: false;
			...
			connection = ModelConnectionPersistence.createConnection(clientID,
					host, port, context, ssl, null);
			connection.addConnectionOptions(opts);
			connection.assignPersistenceId(id);
			connection.setCtxId(ctx_id);
	 ...
	 * 
	 * @param connection
	 *            The connection to delete from the database
	 */
	public void deleteConnection(ModelConnectionPersistence connection) {
		SQLiteDatabase db = getWritableDatabase();
